...
const uint8_t token_public_key[] = {0x04,0xff,0x21,0xf8,0xe6,0x4d,0x3a,0x3c,0x01,0x98,0xed,0xfb,0xb7,0xaf,0xdc,0x79,0xbe,0x95,0x94,0x32,0xe9,0x2e,0x2f,0x8a,0x19,0x84,0xbb,0x43,0x6a,0x41,0x4b,0x8e,0xdc,0xec,0x03,0x45,0xaa,0xd0,0xc1,0xbf,0x7d,0xa0,0x4f,0xd0,0x36,0xdd,0x7f,0x9f,0x61,0x7e,0x30,0x66,0x92,0x24,0x28,0x3d,0x95,0x0f,0xab,0x9d,0xd8,0x48,0x31,0xdc,0x83};
...
int verifyTokenNameID(uint8_t *tokenId, uint8_t *tokenName, uint8_t decimals, uint8_t *signature, uint8_t signatureLength){
    ...
    os_memmove(buffer, tokenId, strlen(tokenId));
    os_memmove(buffer+strlen(tokenId), tokenName, strlen(tokenName));
    os_memmove(buffer+strlen(tokenId)+strlen(tokenName), &decimals, 1);
